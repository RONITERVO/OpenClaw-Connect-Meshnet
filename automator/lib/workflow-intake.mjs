...
- When useSubagents is true, treat spawned subagents as side-effect-free advisors. They may research, critique, fact-check, brainstorm, compare, inspect, or review, but the parent agent decides, fixes valid critique, owns all side effects, and reports state.
...
function workflowIntakeSchema() {
  return {
    ...
    safety: [
      ...
      "Subagent-ready workflows use advisory subagents: parent agents decide, fix valid critique, own side effects, and report state before progress or completion.",
    ...
    request: {
      ...
      useSubagents: "Optional boolean. When true, Automator makes the cron agent-turn subagent-ready by adding advisory-review guidance to the prompt and merging agents_list, sessions_spawn, sessions_yield, and subagents into an explicit --tools allow-list when one is supplied. Subagents advise only; the parent validates critique, performs any side effects, and reports workflow state.",
...
function workflowIntakeDocs() {
  ...
  return [
    ...
    "- For subagent-ready workflows, set useSubagents:true. Subagents are advisory reviewers: they may research, critique, fact-check, brainstorm, compare, inspect, or review, but the parent validates findings, fixes valid critique, owns all side effects, and reports COMPLETE or PROGRESS only after that review.",
