...
- When useSubagents is true and sessions_spawn is available, advisory subagent review is required. Spawn multiple side-effect-free reviewers before finalizing work, with at least correctness/safety, completeness/user-intent, and quality/edge-case lanes when practical. The parent agent decides, fixes valid critique, owns all side effects, and reports state.
...
function workflowIntakeSchema() {
  return {
    ...
    safety: [
      ...
      "Subagent-ready workflows require advisory review when sessions_spawn is available: parent agents spawn multiple side-effect-free reviewers when practical, decide, fix valid critique, own side effects, and report state before progress or completion.",
...
function workflowIntakeDocs() {
  ...
  return [
    ...
    "- For subagent-ready workflows, set useSubagents:true. When sessions_spawn is available, advisory review is required: spawn multiple side-effect-free reviewers before finalizing work, with at least correctness/safety, completeness/user-intent, and quality/edge-case lanes when practical. Subagents may research, critique, fact-check, brainstorm, compare, inspect, or review, but the parent validates findings, fixes valid critique, owns all side effects, and reports COMPLETE or PROGRESS only after that review.",
