...
- Use **Workflow state** for scheduled jobs that should start from a precise current step. The app can append a compact static state block, or use **Step plan controller** for one repeating cron that advances through configured steps only after the agent reports the current step complete. Step controller jobs need the Automator backend running on `127.0.0.1:18890` when the agent reports progress.
