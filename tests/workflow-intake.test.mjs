...

{
  ...
  assert.match(previewIntake.warnings.join("\n"), /correctness\/safety, completeness\/user-intent, and quality\/edge-case/);
  ...
  assert.match(previewIntake.controllerMessagePreview, /Advisory subagent review is required/);
  assert.match(previewIntake.controllerMessagePreview, /Spawn at least three distinct advisory reviewers when practical/);
  assert.match(previewIntake.controllerMessagePreview, /native read-only advisory reviewers/);
  assert.match(previewIntake.controllerMessagePreview, /Do not call sessions_yield after failed or unavailable spawns/);
  assert.match(previewIntake.controllerMessagePreview, /fix every valid critique that affects correctness, safety, user intent, continuity, completeness, or quality before reporting PROGRESS or COMPLETE/);
